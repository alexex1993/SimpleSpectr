...
3. **`SpectrogramEngine`** — pure, `nonisolated` static functions (must stay off the MainActor; the target sets `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`, so types are main-isolated unless marked otherwise). Pipeline: `generate` opens the file and runs a streaming STFT (Hann window, real FFT `vDSP_fft_zrip`, amplitude-normalized magnitude → dB) via Accelerate `vDSP` → `makeImage` (dB → color via `Colormap`, packed RGBA → `CGImage`). A `MonoSource` private struct reads the file forward chunk-by-chunk and down-mixes to mono on demand, so the whole file is never held in memory; `Task.checkCancellation()` is honored between columns. Returns a `SpectrogramResult` (`@unchecked Sendable` because it carries a `CGImage`).
...
5. **`AudioPlayerController`** (`@MainActor` `ObservableObject`) — plays back the loaded file (`AVAudioPlayer` initialized from the file `Data`) and publishes the playhead position (`currentTime`). `ContentView` owns it as a `@StateObject` and loads/unloads it via `.onChange(of: loadedURL)`. A `Timer` on the main run loop (common mode) polls `AVAudioPlayer.currentTime` at ~60 fps while playing to drive the synced playhead; natural end-of-file is detected by `isPlaying && !player.isPlaying`. **`PlayerBar`** (in `ContentView`) shows play/pause, current/total time, a seek slider, and a mute toggle. **`SpectrogramScene`** draws an accent-colored playhead line from `currentTime/result.duration` and seeks on click/drag (the spectrogram cursor is the navigation surface).
...
- **AVAudioFile reading**: `read(into:frameCount:)` *throws* at EOF instead of returning 0 frames. The chunked read loop in `MonoSource.decodeNextChunk` is driven by `framePosition < length` and reads `min(chunk, remaining)` — do not switch it to a `while true` + zero-frame-break loop, that crashes.
- **Task cancellation**: `SpectrogramModel.load` stores and cancels the previous `Task`; `generate` checks `Task.isCancelled` each column. `SpectrogramError.invalidFFTSize` guards that `fftSize` is a power of two.
...
- **File access**: engine calls `startAccessingSecurityScopedResource()` (balanced in `defer`) since the app is sandboxed read-only for user-selected files. `AudioPlayerController.load` reads the file `Data` inside the security scope, then releases it before playback (the `AVAudioPlayer` keeps the in-memory copy, not the file handle).
- **Player memory**: unlike the streaming engine, the player materializes the whole file (`AVAudioPlayer(data:)`) for instant seeking. The `loaded` state carries the source `URL` precisely so the player can reload it.
